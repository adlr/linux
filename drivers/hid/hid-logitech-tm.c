		...
		struct hidpp_report *hidpp_report)
{
	...
	if (hidpp_report->report_id == 0x02 &&
		(hidpp_report->rap.params[2] || hidpp_report->rap.params[3])) {
		/* Got emulated scroll wheel event. Must be out of raw mode,
		 * so go back into it */
		fd->in_raw_mode = 0;
	}
	if (fd->in_raw_mode == 0) {
		int already_requested = fd->raw_switch_requested;
		spin_unlock_irqrestore(&fd->lock, flags);
		if (already_requested) {
			return 1;  // do nothing more
		}
		dbg_hid("Going to raw mode\n");
